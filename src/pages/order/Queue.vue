<template>
  <div class="q-py-md table-container">
    <div class="row">
      ...
      <h6 class="title q-my-lg">PEDIDOS EN CURSO</h6>
    </div>
                ...
                <template v-slot:body-cell-Acciones="props">
                  <td class="accions-td">
                    <q-btn-group class="full-width full-height" outline square>
                      ...
                      <q-btn
                        v-if="props.row.statusorder == 'PENDIENTE'"
                        icon="check_circle"
                        text-color="blue-10"
                        class="col text-bold q-pa-none icon-table"
                        @click="deliverOrderUser(props.row._id)"
                      >
                        <q-tooltip class="bg-indigo" :offset="[10, 10]">
                    ...
                    <td class="accions-td">
                      ...
                      >
                        <q-btn
                          ...
                          class="col q-pa-none"
                          icon="delete"
                          @click="deleteItemOrder(props.row.codigo)"
                        >
                          <q-tooltip class="bg-indigo" :offset="[10, 10]">
                            Eliminar
                          </q-tooltip>
  ...
  try {
    ...
    if (orders) {
      orders.forEach((item) => {
        ...
        if (item.statusorder == "PENDIENTE") {
          item.id = countActive++;
          rows.value.push(item);
        } else if (item.statusorder == "CANCELADO") {
...
async function deliverOrderUser(id) {
  loading.value = true;
  try {
    await deliverOrder(id, idFarm.value);
    showNotification("positive", "Pedido entregado correctamente");
    loading.value = false;
    rows.value = [];
    inactiveRows.value = [];
    getAllOrders();
    ...
    loading.value = false;
    showNotification("negative", "Ocurrió un error al entregar el pedido");
  }
}
